...
import styles from './ContentPageLayout.module.scss'; // 导入与自己同名的样式文件
...
const ContentPageLayout = ({ pageData, children }) => {
  if (!pageData) {
  ...
  // 为个性化banner样式定义一个辅助函数或直接计算
  const getModifierClass = () => {
    switch (pageData.title) {
      case 'Proof of Concept':
        return styles.oversizeContainer;
      case 'iHP':
        return styles.oversizeContainer;
      default:
        return ''; // 如果没有匹配的标题，则不添加额外 class
    }
  };

  const animationContainerClasses = `${styles.titleAnimationContainer} ${getModifierClass()}`;
    ...
    <div className={styles.layoutWrapper}>
      ...
      <BaseBanner id={`${pageData.title}-banner`} className={styles.descriptionBanner}>
        <div className={styles.bannerContent}>
          {/* 动画 */}
          <div className={animationContainerClasses}>
