...
const ProtocolMd = dynamic(() => import('../md/protocol.md'), { ssr: false });

const ProtocolPage = () => {
  const initialPageData = {
    title: 'Protocol',
    description: "The standard operating procedures used in our lab work, available for other teams to use and adapt.",
    sections: [], // 使用从 markdown 提取的标题数据
  };
...
export async function getStaticProps() {
  const filePath = path.join(process.cwd(), 'static', 'data', 'wet-lab', 'protocol.json');
...
ProtocolPage.getLayout = function getLayout(page, pageProps) {
  return <Layout {...pageProps}>{page}</Layout>;
};

ProtocolPage.layoutOptions = {
  showGrid: false, // 明确指令：不显示 Gri
};
